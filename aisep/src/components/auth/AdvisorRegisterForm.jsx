...
/**
 * AdvisorRegisterForm - Simplified credential collection
 * Only collects: Full Name, Email, Password
 */
function AdvisorRegisterForm({ onBack, onComplete, termsData, onFetchTerms }) {
  ...
  const validateForm = () => {
    ...
    else if (!/^[a-zA-Z0-9]+$/.test(formData.username)) newErrors.username = 'Tên người dùng chỉ được chứa chữ cái và số.';
      ...
      <div className={styles.reg_cardBody}>
        <div className={styles.reg_stepContainer}>
          <div>
            ...
            <p className={styles.reg_stepSubtitle}>Đăng ký với tư cách Cố vấn</p>
          </div>
