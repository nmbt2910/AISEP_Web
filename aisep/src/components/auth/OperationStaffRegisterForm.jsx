...
function OperationStaffRegisterForm({ onBack, onComplete, termsData, onFetchTerms }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        onComplete && onComplete({ ...formData, role: 'OperationStaff' });
      } else {
      ...
      <div className={styles.reg_cardBody}>
        <div className={styles.reg_stepContainer}>
          <div>
            ...
            <p className={styles.reg_stepSubtitle}>Đăng ký với tư cách Nhân viên vận hành</p>
          </div>
...
export default OperationStaffRegisterForm;
